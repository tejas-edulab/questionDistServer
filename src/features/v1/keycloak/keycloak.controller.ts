...
import { keycloakRegisterSchema, keycloakAssignClientRollToUserSchema, keycloakCreateRoleSchema, keycloakUpdateUserSchema } from "./keycloak.validator";
...
export default class KeycloakController {
  ...
  public static createClientRole = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      try {
        ...
        const roleData = getRole.filter((currentRole: IKeycloakRole) => currentRole.name === name);
  ...
  static registerUser = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const { firstName, lastName, email, password, roleInfo, username } = await keycloakRegisterSchema.validateAsync(req.body);
      ...
      // Create User in our database
      const userInfo = await UserRepository.addUser(firstName, lastName, email, username || email);
      ...
      try {
        ...
        console.log("Reached in catched");
      ...
      */
      try {
        ...
        console.log("Reached in catch here");
        logger.error(e);
        return res.status(500).json({ status: 500, error: "Error while assigning role. Assigned Default Role", message: "User Created with default role" });
      }
  ...
  //         user.userId = userId;
  //         await UserRepository.updateUser(user.email, user.firstName, user.lastName, user.role, userId);
  //       } catch (e) {
  //         logger.error(e);
  ...
  //             const keycloakRoleData: IKeycloakRole[] = keycloakRoles.filter((currentRole: IKeycloakRole) => currentRole.name === value.role);
  ...
  static updateUser = async (req: Request, res: Response, next: NextFunction) => {
    try {
      /*
        Validate request body
      */
      const {email, roleInfo, firstName, lastName} = await keycloakUpdateUserSchema.validateAsync(req.body);

      const superAdminContain = roleInfo.filter((obj)=>obj.role === IRoles.SUPER_ADMIN)

      if(superAdminContain.length > 0)  return next(ApiError.customError(401,'You Are Not Authorize For Change SUPER_ADMIN Role!'))

      let userId = '';
      const user = await UserRepository.findUserByEmail(email);
      if (!user) return next(ApiError.customError(400, 'User Not Found'));
      if (user.userId === null ) {
        const checkUser = await KeycloakApi
          .getKeycloakUser(email, req.headers.authorization);
      ...
      /*
       ...
       If entry found return error
      */
     const roleCheck =await Promise.all(roleInfo.map(async(obj)=>{
      const registeredUser = await RoleRepositories.fetchRoleByRoleName(obj.role);
      if (!registeredUser) return next(ApiError.customError(404, ` ${obj.role} Role not found`));
      return  { id: registeredUser.roleId, name: obj.role }
     }))
     const keycloakUserIds = roleCheck.map((obj)=>obj.id)
    ...
    await UserRepository.updateUser(
      ...
      */
      ...
      try {
      await KeycloakApi.updateUser(data, userId, req.headers.authorization);
      await KeycloakApi.deleteRolesForUser(user.userId,user.email,req.headers.authorization)

      const keyclaokUpdateRole = await KeycloakApi.assignRole(roleCheck, userId, req.headers.authorization);
      if(keyclaokUpdateRole){
      const userRole = await UserRoleRepository.fetchUserRoleById(user.id)
      if(!userRole) return next(ApiError.customError(404,'User Role Not found'))
      await UserRoleRepository.deleteUserRoleByUserId(user.id)
      const updateUserRole = roleInfo.map((obj)=>{ return  {...obj,userId:user.id} })
      await UserRoleRepository.createUserRole(updateUserRole)
      }
   
      } catch (e) {
        console.log(e.message)
        // -todo- restore original role
        await UserRepository.updateUser(
          user.email,
          user.firstName,
          user.lastName,
